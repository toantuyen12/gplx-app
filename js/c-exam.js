// Hạng C: 40 câu, 24 phút, Đạt >= 36/40, không sai câu điểm liệt
...
const EXAM_DURATION = 24 * 60; // 24 minutes
const RECENT_KEY = 'c_exam_recent';
const HISTORY_KEY = 'c_exam_history';
...
// Exam generation rule (C):
// 10 Ch1 + 1 Ch2 + 2 Ch3 + 1 Ch4 + 14 Ch5 + 11 Ch6 + 1 critical = 40
const EXAM_RULE = {
  ch1NonCrit: 10,
  ch2: 1,
  ch3: 2,
...
// Max unique questions across 5 exams = 40 * 5 = 200
...
document.addEventListener("DOMContentLoaded", () => {
    document.title = `Thi Đề 40 Câu GPLX Hạng C Online | thigplx.site`;
    const label = document.getElementById("pageTitleLabel");
    if (label) label.textContent = `Thi Thử Sát Hạch Hạng C`;
});
...
async function loadData() {
    try {
        ...
        // Use 'default' (600-question) mode for Hạng C
        _manager = createQuestionManager('default', allQuestionsRaw, allExplanationsRaw, 'c');
...
// ===== EXAM GENERATION =====
function generateCExam() {
    console.log("--- C Exam Generation Started ---");
    let recentIds = [];
    try {
...
function startExam() {
    ...
    document.getElementById("cExamRoot").style.display = "block";
...
window.exitHome = function() {
    window.location.href = 'class-c-menu.html';
};
...
// ===== RENDER GRID =====
function renderGrid() {
    const grid = document.getElementById('cExamGrid');
    if (!grid) return;
...
function renderQuestion() {
    const area = document.getElementById('cExamQuestionArea');
    if (!area) return;
...
function renderResult(score, isPass, wrongCritical) {
    ...
    <div class="result-summary" style="padding:20px;background:#fff;border-radius:12px;margin-bottom:20px;box-shadow:0 4px 6px -1px rgba(0,0,0,0.1);text-align:center;">
      <h2 style="color:#1e293b;margin-top:0;margin-bottom:15px;font-size:24px;">KẾT QUẢ THI LÝ THUYẾT HẠNG C</h2>
...
document.addEventListener("keydown", function(event) {
    if (!document.getElementById("cExamRoot") || document.getElementById("cExamRoot").style.display === "none") return;
