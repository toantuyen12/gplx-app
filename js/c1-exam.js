...
async function loadData() {
    try {
        ...
        if (_manager && quiz.length > 0) {
            ...
            const root = document.getElementById("c1ExamRoot");
...
function generateC1Exam() {
    console.log("--- C1 Exam Generation Started ---");
    let recentIds = [];
    try {
    ...
    if (pCrit.length < nc || pCh1.length < n1 || pCh2.length < n2 || 
        ...
        console.warn("Recent list too saturated for C1, resetting.");
        recentIds = [];
        pCrit = criticalPool;
        pCh1 = ch1NonCrit;
        pCh2 = ch2NonCrit;
    ...
    if (quiz.length !== EXAM_TOTAL || finalCritCount !== 1) {
        ...
        if (!window.regenC1) window.regenC1 = 0;
        window.regenC1++;
        if (window.regenC1 > 10) {
            quiz = shuffle([...criticalPool.slice(0,1), ...ch1NonCrit.slice(0, EXAM_TOTAL-1)]);
            window.regenC1 = 0;
            return;
        }
        return generateC1Exam();
    }
    window.regenC1 = 0;
