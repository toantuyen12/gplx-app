...
// ===== CONSTANTS =====
const EXAM_TOTAL = 30;
const PASSING_SCORE = 27;
const EXAM_DURATION = 20 * 60; // 20 minutes
const RECENT_KEY = 'b_exam_recent';
const HISTORY_KEY = 'b_exam_history';

// Chapter ranges for the 600-question default mode
// Ch1: 1-180, Ch2: 181-205, Ch3: 206-263, Ch4: 264-300, Ch5: 301-485, Ch6: 486-600
...
// Exam generation rule: 8 Ch1 + 1 Ch2 + 1 Ch3 + 1 Ch4 + 9 Ch5 + 9 Ch6 + 1 critical = 30
// Note: critical comes from the is_critical pool (any chapter), so Ch1 pool is split
const EXAM_RULE = {
  ch1NonCrit: 8,
  ch2: 1,
  ch3: 1,
...
// Max unique questions across 5 exams = 30 * 5 = 150
...
let quiz = []; // Array of newIds (1-600 in 'default' mode) 
let userAns = [];
let current = 0;
let timeLeft = 0;
...
document.addEventListener("DOMContentLoaded", () => {
    document.title = `Thi Đề 30 Câu GPLX Hạng B Online | thigplx.site`;
    const label = document.getElementById("pageTitleLabel");
    if (label) label.textContent = `Thi Thử Sát Hạch Hạng B`;
});
...
async function loadData() {
    try {
        ...
        // Use 'default' (600-question) mode for Hạng B
        _manager = createQuestionManager('default', allQuestionsRaw, allExplanationsRaw, 'b');
...
function getQuestionsByChapter(chapterId) {
    return _manager.chapters.find(c => c.id === chapterId)
        ? _manager.chapters.filter(c => c.id === chapterId).flatMap(() =>
            Array.from({ length: 600 }, (_, i) => i + 1).filter(id => {
                const m = _manager.getMapping(id);
                return m && m.chapterId === chapterId;
            })
        )
        : [];
}
...
// ===== EXAM STATE =====
function startExam() {
    // Vignette + cho phép onclick của click này, sau đó mới bật shield
...
// ===== RENDER GRID =====
function renderGrid() {
    const grid = document.getElementById('bExamGrid');
    if (!grid) return;
...
function renderQuestion() {
    const area = document.getElementById('bExamQuestionArea');
    if (!area) return;
...
function retryExam() {
    if (window.showAd) window.showAd();
    // Cho phép onclick của click này, sau đó mới bật lại shield
    setTimeout(function() {
        if (window.MoneytagAds) window.MoneytagAds.setExamMode(true);
    }, 100);
    isSubmitted = false;
